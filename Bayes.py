...
def run(data, distancia):
    ...
    colors = plt.cm.tab10.colors  # Cores da paleta de cores tab10
    for i, label in enumerate(y_test.unique()):
        axes[0].scatter(X_pca[y_test == label, 0], X_pca[y_test == label, 1], label=f'Classe {label}', color=colors[i])
    ...
    # Salvar a imagem
    # plt.savefig(f'analise_NB_{distancia}.png')
